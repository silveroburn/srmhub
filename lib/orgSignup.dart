...
Future<int> orgCheck (String name, String email, String password, BuildContext context) async {
  final url = Uri.parse('http://192.168.65.11:3000/organizerSignup');

  final result = await http.post(
    url,
    headers: {'Content-Type' : 'Application/json'},
    body: json.encode({
      'name': name,  // Changed from 'username' to 'name' to match server
      'email': email,
      'password': password
    })
  );
  if (result.statusCode == 200){
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Organizer signup successful')));
    return 1;
  }
  else{
    final m = json.decode(result.body);
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(m['message'])));
    return 0;
  }
}
            ...
            children: [
              ...
              MaterialButton(
                onPressed: () async {  
                  ...
                  var result = await orgCheck(username, email, password, context);
                  // if (result == 1){  // Changed from 0 to 1 to navigate on success
                  //   Navigator.push(context,
                  //   MaterialPageRoute(builder: (context) => const Orgsignin()));
                  // }
                },
